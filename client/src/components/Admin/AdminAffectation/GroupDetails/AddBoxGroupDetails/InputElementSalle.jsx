...
function InputElementSalle({ setId_salle, title, url }) {
    const [inputValue, setInputValue] = useState("")
    const [itemdata, setItemdata] = useState([])
    const getData = async (value) => {
        const res = await axios.get(url)
        console.log(res.data)
        const results = res.data.filter((salle) => {
            return (
                value &&
                salle &&
    ...
    return (
        <div className=' w-full'>
            ...
            />
            <div className="rounded-md py-4 absolute left-60 bottom-16 bg-sky-100 h-56 overflow-auto scrollbar">
                {
                    itemdata.map((item) => (
                        <div
...
export default InputElementSalle