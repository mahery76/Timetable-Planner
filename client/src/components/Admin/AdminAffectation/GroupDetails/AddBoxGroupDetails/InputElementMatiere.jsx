...
function InputElementMatiere({ setId_matiere, title, url }) {
    const [inputValue, setInputValue] = useState("")
    const [itemdata, setItemdata] = useState([])
    const getData = async (value) => {
        const res = await axios.get(url)
        console.log(res.data)
        const results = res.data.filter((matiere) => {
            return(
                value &&
                matiere && 
                matiere.nom_matiere.toLowerCase().includes(value.toLowerCase()))
        })
        setItemdata(results)
    }
            ...
            <div className="rounded-md py-4 absolute left-64 bottom-16 bg-sky-100 h-56 overflow-auto scrollbar">
                {
                    itemdata.map((item) => (
                        <div
                            ...
                            onClick={
                                ()=> {
                                    setInputValue(item.nom_matiere)
                                    setId_matiere(item.id_matiere)
                                    setItemdata([])
                                    console.log(item.id_matiere)
                                }
                            }
                        >
                            {item.nom_matiere}
...
export default InputElementMatiere