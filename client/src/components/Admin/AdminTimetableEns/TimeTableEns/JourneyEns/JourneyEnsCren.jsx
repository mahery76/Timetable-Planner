...
function JourneyEnsCren({ id_cren, date }) {
  ...
  const getOccupations = async () => {
    if (id_ens) {
      const res = await axios.get(`${import.meta.env.VITE_APP_API_URL}/occupationsEns?id_ens=${id_ens}&id_cren=${id_cren}`)
  ...
  const handleDelete = async (id_occupation) => {
    if(!isDone){
      if (window.confirm(`Voulez vous supprimer le seance ${id_occupation}`)) {
        const resp = await axios.get(`${import.meta.env.VITE_APP_API_URL}/deleteOccupation/${id_occupation}`)
        setMatiere("")
        setClasse("")
        setClasse2("")
        setClasse3("")
  ...
  return (
    ...
    >
      <div className='font-bold text-xs flex justify-center text-center w-full my-2'>
        {matiere}
      </div>
