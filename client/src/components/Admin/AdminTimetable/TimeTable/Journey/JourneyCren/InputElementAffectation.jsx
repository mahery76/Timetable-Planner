...
function InputElementAffectation({ setId_matiere, setId_ens, setId_tronc_commun, title }) {
  ...
  const getData = async (value) => {
    try {
      const res = await axios.get(`${process.env.REACT_API_URL}/GroupAffectation/${id_classe}`)
  ...
  const handleChoose = async (choosedValue, id_affectation) => {
    const res = await axios.get(`${process.env.REACT_API_URL}/Affectation/${id_affectation}`)
