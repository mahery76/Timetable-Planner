...
function Timetable() {
  ...
  const getClasseName = async () => {
    if (id_classe) {
      const currentClasse = await axios.get(`${import.meta.env.VITE_APP_API_URL}/group/${id_classe}`)
  ...
  const handleGenerate = async (startDay, endDay) => {
    ...
    `)) {
      ...
      const res = await axios.get(`${import.meta.env.VITE_APP_API_URL}/genOccupation?dateDebut=${startDay}&dateFin=${endDay}`)
  ...
  const handlCheckTimetable = async (startDay, endDay) => {
    ...
    ${FrDate(weekDays[1])} au ${FrDate(weekDays[weekDays.length - 1])}
    comme terminées`)) {
      const res = await axios.get(`${import.meta.env.VITE_APP_API_URL}/checkTimetable?dateDebut=${startDay}&dateFin=${endDay}`)
      // window.location.reload(false);
      setIsGenerated(() => !isGenerated)
    }
  }
  const handleDeleteTimetable = async (startDay, endDay) => {
    ...
    ${FrDate(weekDays[1])} au ${FrDate(weekDays[weekDays.length - 1])}
    `)) {
      const res = await axios.get(`${import.meta.env.VITE_APP_API_URL}/deleteTimetable?dateDebut=${startDay}&dateFin=${endDay}`)
      // window.location.reload(false);
      setIsGenerated(() => !isGenerated)
    }
  }
